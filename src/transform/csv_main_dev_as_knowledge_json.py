...
def _insert_parts_into(hierarchy, module, knowledge, parts):
    ...
    return _insert_parts_into(
        existing_branch["children"], module, knowledge, parts=parts[1:]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--owners",
        required=True,
        help="A CSV file generated by a Code Maat main-dev analysis",
    )
    parser.add_argument(
        "--authors",
        required=True,
        help="A CSV file specifying the color to use for each author",
    )
