...
class Merged(object):
    ...
    def sorted_result(self):
        ...
        ordered = sorted(
            self._merged.items(), key=lambda item: item[1][0], reverse=True
        )
        return ordered
...
class WeightCalculator(object):
    ...
    def __init__(self, half_life, analysis_results):
        ...
        self._colored_weights = dict(
            [(name, as_colored_weight(n)) for name, n in analysis_results]
        )
...
def run(args):
    raw_weights = parse_csv(
        args.weights, parse_action=make_element_weight_parser(args.weightcolumn)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--structure", required=True, help="A CSV file generated by cloc"
    )
    parser.add_argument(
        "--weights",
        required=True,
        help="A CSV file with code age results from Code Maat",
    )
    parser.add_argument(
        "--weightcolumn",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Specifies the half life for a module in months",
    )
